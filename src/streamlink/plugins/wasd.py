...
class WASD(Plugin):
	...
	def _get_streams(self):
		...
		if re_video_id:
			streams = self.getStreamsFromURL("https://wasd.tv/api/media-containers/" + re_video_id.group(1), 'video')
		else:
			streams = self.getStreamsFromURL("https://wasd.tv/api/media-containers?media_container_status=RUNNING&limit=1&offset=0&channel_id=" + re_channel_id.group(1) + "&media_container_type=SINGLE,COOP", 'channel')
