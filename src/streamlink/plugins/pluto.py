...
class Pluto(Plugin):
    url_re_live = re.compile(r'^https?://(?:www\.)?pluto\.tv/live-tv/(?P<slug>[a-zA-Z0-9()\.-]+)$')
    url_re_tv = re.compile(r'^https?://(?:www\.)?pluto\.tv/on-demand/series/(?P<series_slug>[a-zA-Z0-9()\.-]+)/season/\d+/episode/(?P<episode_slug>[a-zA-Z0-9()\.-]+)$')
    url_re_movie = re.compile(r'^https?://(?:www\.)?pluto\.tv/on-demand/movies/(?P<slug>[a-zA-Z0-9()\.-]+)$')
