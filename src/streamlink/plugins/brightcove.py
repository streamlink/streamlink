...
class BrightcovePlayer(object):
    ...
    def __init__(self, session, account_id, player_id="default_default"):
        ...
        log.debug("Creating player for account {0} (player_id={1})", account_id, player_id)
    ...
    def get_streams(self, video_id):
        log.debug("Finding streams for video: {0}", video_id)
        policy_key = self.policy_key(video_id)
        log.debug("Found policy key: {0}", policy_key)
