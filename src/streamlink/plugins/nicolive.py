...
class NicoLiveWsClient(WebsocketClient):
    ...
    def __init__(self, *args, **kwargs):
        super(NicoLiveWsClient, self).__init__(*args, **kwargs)
        self.opened = Event()
        self.ready = Event()

    def on_open(self, wsapp):
        super(NicoLiveWsClient, self).on_open(wsapp)
        self.send_playerversion()
        self.send_getpermit()

    def on_message(self, wsapp, data):
        log.debug(u"Received: {0}".format(data))
        ...
        if msgtype == "ping":
            ...
            log.info("Received disconnect message: {0}".format(reason))
            self.close()
...
class NicoLiveHLSStreamReader(HLSStreamReader):
    def open(self):
        ...
        super(NicoLiveHLSStreamReader, self).open()

    def close(self):
        super(NicoLiveHLSStreamReader, self).close()
...
class NicoLiveHLSStream(HLSStream):
    ...
    def set_wsclient(self, wsclient):
...
class NicoLive(Plugin):
    ...
    def _get_hls_stream_url(self):
        log.debug("Waiting for permit (for at most {0} seconds)...".format(self.STREAM_READY_TIMEOUT))
    ...
    def niconico_web_login(self):
        ...
        if user_session is not None:
            ...
            for elem in root.xpath(".//form[@action]//input"):
                if elem.attrib.get("value"):
                    ...
                    if elem.attrib.get("id") == "oneTimePw":
                        ...
                        log.debug("unknown input: {0}".format(elem.attrib.get('name')))
            ...
            if self.session.http.cookies.get("user_session") is None:
                ...
                log.warning("Login failed: {0}".format(error or 'unknown reason'))
            else:
                log.info("Logged in.")
                self.save_cookies()
