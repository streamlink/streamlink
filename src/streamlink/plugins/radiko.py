...
class Radiko(Plugin):
    _url_re = re.compile(r'http://radiko\.jp/(#!/)?(?P<state>live|ts)/(?P<station_id>\w+)/?(?P<start_at>\d+)?')
    ...
    def _live(self, station_id):
        live_url = 'https://rpaa.smartstream.ne.jp/so/playlist.m3u8'
    ...
    def _get_xml(self, start_at, station_id):
        ...
        api = 'http://radiko.jp/v3/program/station/date/{}/{}.xml'.format(
            date, station_id)
