...
class DenoSolver(Solver):
    def _solve(self, js_input: str) -> str | None:
        deno = DenoProcessor(stdin=js_input)
        deno.run()
        return deno.output
...
class YoutubeAPI:
    ...
    def _extract_hls(self, player_responses: list[dict], player_url: str) -> str:
        slvr: Solver = DenoSolver(self.session)
        ...
        for response in player_responses:
            ...
            if n_matches and yt_dlp_solver is None:
                log.debug("External JS for n-challenge solving not found. Skipping solving n-challenge %r", n_matches[0])

            elif n_matches:
                n_token = n_matches[0]
                log.trace("Solving n-challenge token: %s", n_token)
                result = slvr.solve(NChallengeInput(token=n_token, player_url=player_url))
                if result.solved is None and isinstance(slvr, DenoSolver):
                    log.warning("Could not solve the n-challenge using Deno. Trying to solve using CDP")
                    slvr = CDPSolver(self.session)
                    result = slvr.solve(NChallengeInput(token=n_token, player_url=player_url))

                if result.solved:
                    hls_manifest_url = hls_manifest_url.replace(f"/n/{n_token}", f"/n/{result.solved}")
    ...
    def get_streams_from_api(self, url, video_id) -> str | None:
...
class YouTube(Plugin):
    ...
    def _get_streams(self):
        ...
        try:
            hls_url = api.process_watch_page(self.url, self.match["video_id"])
            if hls_url:
            ...
            log.error("Failed finding data, falling back to YouTube API: %s", e)
            hls_url = api.get_streams_from_api(self.url, self.match["video_id"])
            if hls_url:
                return HLSStream.parse_variant_playlist(self.session, hls_url, start_offset=self.time_offset, name_key="pixels")
            return None

        finally:
            if not any([api.id, api.author, api.category, api.title]):
                api.extract_metadata(url=self.url)
            self.id, self.author, self.category, self.title = api.id, api.author, api.category, api.title
