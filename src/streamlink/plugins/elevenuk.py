...
class UFCTV(Plugin):
    url_re = re.compile(r"https?://(?:www\.)?elevensports\.uk/(channel|video)/.+")
    video_info_re = re.compile(r"""program\s*=\s*(\{.*?});""", re.DOTALL)
    channel_info_re = re.compile(r"""g_channel\s*=\s(\{.*?});""", re.DOTALL)

    stream_api_url = "https://www.elevensports.uk/service/publishpoint"
    auth_url = "https://www.elevensports.uk/secure/authenticate"
