...
class DLive(Plugin):
    ...
    def stream_weight(cls, key):
        ...
        return super(DLive, cls).stream_weight(key)

    def _get_streams_video(self, video):
        log.debug("Getting video HLS streams for {0}".format(video))
    ...
    def _get_streams_live(self, channel):
        log.debug("Getting live HLS streams for {0}".format(channel))
        query = """query {{
            userByDisplayName(displayname:"{0}") {{
                livestream {{
                    title
                }}
                username
