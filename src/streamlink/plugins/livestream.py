...
URL_API_EVENT = "https://api.new.livestream.com/accounts/{}/events/{}"
...
class Livestream(Plugin):
    ...
    def _get_streams(self):
        ...
        if self.url.__contains__("api.new."):
            ...
            account_no = self.match.group(1)
            events = self.session.http.get(URL_API.format(account_no)).json()
            # API verification from given URL if there's a live event / working VOD stream.
            if "data" not in events:
                log.debug("No stream has been found with this URL.")
                return
            stream_no = events["data"][0]["id"]
            res = self.session.http.get(URL_API_EVENT.format(account_no, stream_no)).json()
            stream_info = res["stream_info"]
