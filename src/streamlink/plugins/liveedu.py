...
class LiveEdu(Plugin):
    ...
    def login(self):
        ...
        if email and password:
            ...
            log.debug("Attempting login as {0} (token={1})", email, token)
            ...
            if res.status_code != 302:
                log.error("Failed to login to LiveEdu account: {0}", email)
    ...
    def _get_streams(self):
        ...
        if config["selectedVideoHID"]:
            log.debug("Found video hash ID: {0}", config["selectedVideoHID"])
            api_url = urljoin(self.url, urljoin(config["videosURL"], config["selectedVideoHID"]))
        elif config["livestreamURL"]:
            log.debug("Found live stream URL: {0}", config["livestreamURL"])
        ...
        if "error" in viewing_urls:
            log.error("Failed to load streams: {0}", viewing_urls["error"])
        else:
