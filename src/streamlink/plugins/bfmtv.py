...
class BFMTV(Plugin):
    ...
    def _get_streams(self):
        ...
        if m:
            video_id = m.group('video_id')
            log.debug('Video ID: {0}'.format(video_id))
            for s in self.session.streams(self._dailymotion_url.format(video_id)).items():
                yield s
            return
        ...
        if m:
            log.debug('JS URL: {}'.format(urljoin(self.url, m.group(1))))
